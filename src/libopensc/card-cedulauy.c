 ...
 *
 * The contactless interface is a PACE-protected ICAO/eID interface.  Its
 * EF.CardAccess announces a single PACEInfo, id-PACE-ECDH-GM-AES-CBC-CMAC-256
 * over NIST P-384, and no ChipAuthentication or TerminalAuthentication info,
 * so plain PACE is sufficient and no EAC (TA/CA) is involved.  The card seems to
 * accept the MRZ as the only PACE password: MSE:Set AT with the CAN
 * reference (83 01 02) is rejected with 6A88, the MRZ reference (83 01 01)
...
#define CEDULAUY_SM_OVERHEAD  (4 + 2 + CEDULAUY_SM_MAC_LEN + 2 + 3)
...
static const struct sc_atr_table cedulauy_atrs[] = {
	...
	/* Contactless (NFC) interface of the "IAS Classic v5" card.  The card
	 * randomises four of its historical bytes on every activation
	 * (anti-tracking) and the check byte follows from them, so an exact ATR
	 * is useless here: only the ISO 14443 header and the ATR length are
	 * matched, and cedulauy_match_card() confirms the card by reading
	 * EF.CardAccess, which is the one file readable before PACE. */
...
struct cedulauy_drv_data {
	...
	size_t mrz_len;
	/* the ISO SM handler this driver wraps, see cedulauy_get_sm_apdu() */
	int (*iso_get_sm_apdu)(struct sc_card *card, struct sc_apdu *apdu,
			struct sc_apdu **sm_apdu);
};
...
/*
 * Select the MF with a plain APDU.
 *
 * sc_select_file() cannot be used for this: cedulauy_select_file() answers a
 * request for the MF from a synthetic file object without touching the card,
 * since everything the driver otherwise needs lives inside the application DF.
 */
static int
...
/*
 * Protected response sizing.
 *
 * sm_encrypt() derives the case of the SM APDU from the case of the plain one:
 * a short APDU stays short, and its protected response is then sized for a
 * short APDU and capped at 256 bytes.  The card answers PSO: COMPUTE DIGITAL
 * SIGNATURE with 291 bytes, which does not fit:
 *
 *   87 82 01 11 <indicator> <cryptogram>    277   (the 256 byte signature,
 *   99 02 <SW1> <SW2>                         4    padded to 272 bytes of
 *   8E 08 <MAC>                              10    AES blocks)
 *
 * The plain APDU asks for exactly 256 bytes, so neither sc_detect_apdu_cse()
 * nor sc_format_apdu_cse_lc_le() promotes it to extended length, and asking for
 * extended length explicitly is not an option either: PN53x based readers such
 * as the ACR122U wrap every APDU into a PN532 InDataExchange frame whose length
 * is a single byte, so they cannot carry an extended APDU at all.
 *
 * The response therefore has to arrive in two parts, and for that the SM APDU
 * needs a buffer large enough to reassemble it into, plus an Le that
 * sc_get_response() reads as "data expected".  Both are corrected here, after
 * the ISO SM layer has encrypted and signed the command, so the cryptogram and
 * the MAC are untouched.  See cedulauy_get_response() for the other half.
 */
static int
cedulauy_get_sm_apdu(struct sc_card *card, struct sc_apdu *apdu,
		struct sc_apdu **sm_apdu)
{
	struct cedulauy_drv_data *drv_data = DRVDATA(card);
	size_t resplen;
	...
	if (drv_data == NULL || drv_data->iso_get_sm_apdu == NULL || sm_apdu == NULL)
		LOG_FUNC_RETURN(card->ctx, SC_ERROR_INVALID_ARGUMENTS);

	r = drv_data->iso_get_sm_apdu(card, apdu, sm_apdu);
	if (r < 0 || *sm_apdu == NULL)
		return r;

	/* Everything but PSO: CDS is kept below CEDULAUY_SM_MAX_SIZE, so its
	 * protected form fits into a short response and the ISO SM layer has
	 * already sized it correctly. */
	resplen = CEDULAUY_SM_WRAPPED(apdu->resplen);
	if (resplen <= SC_MAX_APDU_RESP_SIZE || ((*sm_apdu)->cse & SC_APDU_EXT) != 0)
		return r;

	resp = realloc((u8 *)(*sm_apdu)->resp, resplen);
	if (resp == NULL) {
		/* the caller only frees the SM APDU through free_sm_apdu(),
		 * which would try to decrypt a response we never received */
		free((u8 *)(*sm_apdu)->data);
		free((u8 *)(*sm_apdu)->resp);
		free(*sm_apdu);
		*sm_apdu = NULL;
	...
	(*sm_apdu)->resp = resp;
	(*sm_apdu)->resplen = resplen;

	/* sm_encrypt() leaves Le at 0.  For a case 4 short APDU that is encoded
	 * as the Le byte 0x00, which asks the card for 256 bytes, but
	 * sc_get_response() reads the 0 as "no response data expected" and
	 * discards the 61xx remainder.  Spell the same value out instead; the
	 * Le protected inside the cryptogram is built from the plain APDU and
	 * is not affected. */
	if ((*sm_apdu)->le == 0)
		(*sm_apdu)->le = SC_MAX_APDU_RESP_SIZE;

	sc_log(card->ctx,
			"Protected response does not fit into a short APDU, "
			"reserved %"SC_FORMAT_LEN_SIZE_T"u bytes for GET RESPONSE",
			resplen);
...
cedulauy_perform_pace(struct sc_card *card)
{
	...
	/* perform_pace() has just installed the ISO SM handlers; wrap the one
	 * that builds the protected command.  This runs again on every re-PACE,
	 * so guard against wrapping ourselves. */
	if (card->sm_ctx.ops.get_sm_apdu != cedulauy_get_sm_apdu) {
		drv_data->iso_get_sm_apdu = card->sm_ctx.ops.get_sm_apdu;
		card->sm_ctx.ops.get_sm_apdu = cedulauy_get_sm_apdu;
	}
 ...
 *
 * This only matters for the 291-byte protected response to PSO: COMPUTE DIGITAL
 * SIGNATURE, which cannot arrive in one short APDU; see cedulauy_get_sm_apdu()
 * for the buffer it is reassembled into.  Every other command is kept below
 * CEDULAUY_SM_MAX_SIZE so that it never needs chaining at all.
 *
